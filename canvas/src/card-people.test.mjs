...
test("document cards show the last editor, including the creator when they edited last", () => {
  const cardPeople = documentCardPeople({
    ownerAccountId: "owner-account",
    ownerName: "Owner",
    lastEditor: { accountId: "owner-account", name: "Owner", isCurrentUser: true },
  }, currentProfile);

  assert.deepEqual(cardPeople.map((person) => person.accountId), ["owner-account"]);
  assert.equal(cardPeople[0].isCurrentUser, true);
});

test("folder cards include the creator and unique active shared accounts", () => {
  const cardPeople = folderCardPeople({ ownerAccountId: "owner-account", access: "owner" }, [
  ...
  assert.deepEqual(cardPeople.map((person) => person.accountId), ["owner-account", "member-account"]);
});
